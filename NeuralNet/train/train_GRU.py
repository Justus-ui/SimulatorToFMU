...
def train(model: "object", batch_size: "int", epochs: "int", Length_of_sample_s: "float", sr: "float", show_data:bool = False, use_filter:bool = False, minimum_delta: "float" = 0.0) -> "tuple(np.array(float), DataLoader)":
    ...
    for epoch in range(epochs):
        ...
        valid_loss_nd[epoch] = valid_loss
        log.info(f"Validation loss {valid_loss}")
        scheduler.step(valid_loss)
        loss_nd[epoch] = train_loss
    return loss_nd,valid_loss_nd, DataLoader(test_data, batch_size = batch_size, shuffle = True)
...
if __name__ == "__main__":
    ...
    sr = 0.01 #timeinterval[s] between samples
    if sr < 0.0001:
        raise ValueError("sr to small, not enough samples")
    Length_of_sample_s = 0.2 # Window length in seconds. Number of samples will be -> Length_of_samples_s / sr
    ...
    loss_nd,valid_loss_nd, test = train(model, batch_size = batch_size, epochs = epochs, Length_of_sample_s = Length_of_sample_s, sr = sr, show_data = show_data, use_filter = use_filter, minimum_delta=minimum_delta)
    plt.plot(loss_nd, label = "Trainingloss")
    plt.plot(valid_loss_nd, label = "Validationloss")
    plt.title("loss over epochs")
    plt.xlabel("epoch")
    plt.ylabel("Loss [enrms]") 
    plt.legend()
    plt.show()
    #model.save_model()
